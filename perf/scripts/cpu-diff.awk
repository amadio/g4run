...
/duration/ {
	duration[curr_file] = $(NF-1) / 1000.0
}
...
END {
	...
	print ""
	print " CPU Statistics: Differences per Function"
	print ""
	printf("   Before: %.2fs, %.4g cycles, %.4g instructions\n",
		duration[1], total_cycles[1], total_instructions[1])
	printf("    After: %.2fs, %.4g cycles, %.4g instructions\n\n",
		duration[2], total_cycles[2], total_instructions[2])

	printf("  Speedup: %+.2f% (time), %+.2f% (cycles), %+.2f% (instructions)\n",
	...
	for (symbol in symtab) {
		...
		if (ratio_c > 1 - min_diff && ratio_c < 1 + min_diff)
			continue
